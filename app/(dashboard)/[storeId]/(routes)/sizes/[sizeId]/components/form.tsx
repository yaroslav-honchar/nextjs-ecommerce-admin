...
import type { ISizeFormProps } from "@/app/(dashboard)/[storeId]/(routes)/sizes/[sizeId]/components/form.props"
...
export const ClientForm: React.FC<ISizeFormProps> = ({ initialData }) => {
  ...
  const onDeleteCategory = async (): Promise<void> => {
    ...
    try {
      ...
      console.log(error)
      toast.error("Make sure you removed all products with the category first")
    } finally {
      setIsLoading(false)
      setIsOpen(false)
    }
